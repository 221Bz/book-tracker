...
export function useLibraryData() {
  ...
  const [sort, setSort] = useState<"last_read" | "rating" | "title">(
    "last_read"
  );
  ...
  const handleUpdate = async () => {
    ...
    const progress =
      selectedBook.pages > 0
        ? Math.min(
            100,
            Math.round((formData.last_pages / selectedBook.pages) * 100)
          )
        : 0;
  ...
  const toggleFavorite = async (book: UserBook) => {
    ...
    setUserBooks((prev) =>
      prev.map((b) =>
        b.id === book.id ? { ...b, is_favorite: !b.is_favorite } : b
      )
    );
  };

  const handleFormChange = <K extends keyof UserBookForm>(
    field: K,
