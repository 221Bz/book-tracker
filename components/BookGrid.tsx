...
interface BookGridProps {
  ...
  renderStars: (book: UserBook) => React.ReactElement[];
  ...
  return (
    <div className="overflow-y-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6 pr-2">
      {books.map((book) => {
        ...
        return (
          <Card key={book.id} className="rounded-xl bg-[#1C1C1C] hover:bg-[#232323] transition shadow-sm flex flex-col p-0">
            <div className="w-full h-44 flex items-center justify-center bg-transparent rounded-t-2xl overflow-hidden">
              {coverUrl ? (
                <img src={coverUrl} alt={book.title} className="max-h-full max-w-full object-contain" />
              ) : (
            ...
            <CardHeader className="flex flex-col gap-2 pt-2 pb-2 px-3">
              ...
              <p className={`text-sm text-neutral-400 leading-relaxed ${isExpanded ? "" : "line-clamp-3"}`}>
                {book.description}
              </p>
              ...
              <div className="flex items-center gap-2 mt-1">
                <div className="flex gap-1">{renderStars(book)}</div>

                <button onClick={() => onToggleFavorite(book)} className="mt-px">
                  <Heart className={`w-5 h-5 transition ${book.is_favorite ? "fill-pink-500 text-pink-500" : "text-neutral-500 hover:text-pink-400"}`} />
                </button>
              </div>

              <div className="flex flex-wrap gap-2 mt-1">
                {(book.genres ?? []).map((genre) => (
                  <span key={genre} className="bg-gray-100 text-gray-800 text-xs px-2 py-1 rounded-full">{genre}</span>
                ))}
              </div>
            </CardHeader>
            ...
            <CardContent className="space-y-4 pt-0 px-3 pb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 h-2 bg-neutral-400 rounded-full">
                  <div className="h-2 bg-indigo-500 rounded-full" style={{ width: `${book.progress}%` }} />
                </div>
              </div>
